...
public class reportdao {
    ...
    public Map<String, Double> getRevenueByServiceType() throws SQLException {
        ...
        String query = "SELECT s.name AS service_name, SUM(s.cost) AS total_revenue FROM appointments ap JOIN services s ON ap.service_id = s.id JOIN services se ON se.id = ap.service_id WHERE ap.date BETWEEN '2024-07-01' AND '2024-09-01' GROUP BY s.name;";
